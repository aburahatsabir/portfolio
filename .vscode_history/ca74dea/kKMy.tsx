...
const BentoCard: React.FC<{ 
  ...
  subtitle?: string;
  badge?: string;
}> = ({ children, className = "", title, subtitle, badge }) => (
  <motion.div 
    initial={{ opacity: 0, y: 15 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true }}
    className={`bg-white border border-slate-100 rounded-[2.5rem] p-10 shadow-sm hover:shadow-xl transition-all duration-500 group relative overflow-hidden flex flex-col ${className}`}
  >
    ...
    <div className="relative z-10">
      ...
      {title && (
        <div className="mb-8">
          {subtitle && <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-1">{subtitle}</p>}
              ...
              <div className="md:col-span-4 space-y-10 lg:sticky lg:top-10">
                ...
                <div className="pt-8 border-t border-slate-100 flex items-center gap-4">
                  <div className="flex -space-x-3">
                    {[{initials:'MR',color:'bg-blue-600'}, {initials:'SK',color:'bg-slate-700'}, {initials:'DL',color:'bg-blue-800'}].map(item => (
